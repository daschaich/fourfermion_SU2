...
static void flush_lbuf_to_file(gauge_file *gf, fsu2_matrix *lbuf,
  ...
  stat = (int)fwrite(lbuf, 4*sizeof(fsu2_matrix), *buf_length, fp);
  if (stat != *buf_length) {
...
void w_serial(gauge_file *gf) {
  ...
  fsu2_matrix *tbuf = malloc(nx * sizeof(*tbuf));
  ...
  rank29 = sizeof(fsu2_matrix) / sizeof(int32type) * sites_on_node * this_node % 29;
  rank31 = sizeof(fsu2_matrix) / sizeof(int32type) * sites_on_node * this_node % 31;
      ...
      for (y = 0; y < ny; y++) {
        for (x = 0; x < nx; x++, j++) {
          ...
          if (this_node == currentnode) {
            ...
            d2f_gauge(&lattice[i].link[0], &tbuf[tbuf_length]);
          }
...
void r_serial(gauge_file *gf) {
  ...
  if (this_node == 0) {
    ...
    lbuf = malloc(MAX_BUF_LENGTH *4* sizeof(fsu2_matrix));
    if (lbuf == NULL) {
